<?php
	...
	if(!function_exists('__PLHbuildPageXML')){
		function __PLHbuildPageXML($page){
			...
			$current_language = LanguageR::instance()->getLanguage();
			...
			if($children = Symphony::Database()->fetch("SELECT * FROM `tbl_pages` WHERE `parent` = '".$page['id']."' ORDER BY `sortorder` ASC")){
				foreach($children as $c) $oPage->appendChild(__PLHbuildPageXML($c));
			}
	...
	if((!is_array($pages) || empty($pages))){
		...
		foreach($pages as $page) $result->appendChild(__PLHbuildPageXML($page));
	}
